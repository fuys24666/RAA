...
# 通用日志写入函数
# -----------------------------
def log_to_csv(entry: dict, path="logs/training_log_raa_oracle_10_2000_seed42.csv"):
...
def load_dataset(split="train"):
    ...
    for f in files:
        ...
        # 插入 conf_label 为第 6 维（索引5），保持 messages 在最后
        if "conf_label" in data.files:
            c = torch.tensor(data["conf_label"], dtype=torch.float32).unsqueeze(-1)  # [1,T,N,1]
            X = torch.cat([X[..., :5], c, X[..., 5:6]], dim=-1)          # → [1,T,N,7]
...
def main():
    ...
    torch.save({
        ...
        "test_mse": test_mse,
        "test_acc": test_acc
    }, "logs/best_stgnn_raa_oracle_10_2000_seed42.pt")

    print(f"[OK] Saved full model checkpoint → best_stgnn_raa_oracle.pt")
    print(f"Total study time: {(time.time() - study_start)/60:.2f} min")
    # ⭐ 额外：对 test_loader 做一次全量诊断 dump（方便后续画图）
    diag_path = "logs/diagnostics_oracle_10_2000_seed42.npz"
