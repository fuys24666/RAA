...
from model_raa_self import RAASTGNN_self as STGNNModel
...
# 通用日志写入函数
# -----------------------------
def log_to_csv(entry: dict, path="logs/training_log_raa_self_10_2000_seed42.csv"):
...
def load_dataset(split="train"):
    ...
    for f in files:
        ...
        # impact_intensity: [1, N, 1] → 拼起来就是 [B, N, 1]
        impact_list.append(torch.tensor(data["impact_intensity"], dtype=torch.float32))
...
def train_eval(cfg, train_data, val_data, A, device, dist_mat, trial_id=None):
    ...
    print(f"[INFO] Using RAA-self (gat_dim={cfg['gat_dim']}, tf_dim={cfg['tf_dim']})")
...
def main():
    ...
    torch.save({
        ...
        "test_mse": test_mse,
        "test_acc": test_acc
    }, "logs/best_stgnn_raa_self_10_2000_seed42.pt")

    print(f"[OK] Saved full model checkpoint → best_stgnn_raa_self.pt")
    print(f"Total study time: {(time.time() - study_start)/60:.2f} min")
    # ⭐ 额外：对 test_loader 做一次全量诊断 dump（方便后续画图）
    diag_path = "logs/diagnostics_self_10_2000_seed42.npz"
