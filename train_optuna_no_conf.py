...
from model_raa_no_conf import RAASTGNN_NoConf_Transformer as STGNNModel
...
# 通用日志写入函数
# =====================================================
def log_to_csv(entry: dict, path="logs/training_log_raa_noconf_10_2000_seed42.csv"):
...
def load_dataset(split="train"):
    ...
    X_list, y_mask_list, impact_list = [], [], []   # ⭐ 多一个列表

    for f in files:
        data = np.load(os.path.join(folder, f))
        X_list.append(torch.tensor(data["X"], dtype=torch.float32))              # [1,T,N,F]
        y_mask_list.append(torch.tensor(data["wave_mask"], dtype=torch.float32)) # [1,T,N]
        impact_list.append(torch.tensor(data["impact_intensity"], dtype=torch.float32))  # [1,N,1]

    X = torch.cat(X_list, dim=0)          # [B,T,N,F]
    y_mask = torch.cat(y_mask_list, dim=0) # [B,T,N]
    y_impact = torch.cat(impact_list, dim=0) # [B,N,1]
...
def evaluate(model, loader, A, device, dist_mat):
    ...
    with torch.no_grad():
        ...
        for X, y_mask, y_impact in loader:        # ⭐ 多一个 y_impact
            X, y_mask, y_impact = X.to(device), y_mask.to(device), y_impact.to(device)
            pred_I, pred_mask = model(X, A, dist_norm)

            if pred_mask.ndim == 4:
                pred_mask = pred_mask.squeeze(-1) # [B,T,N]
            ...
            acc = early_hit_accuracy(pred_mask, y_mask)
...
def train_eval(cfg, train_data, val_data, A, device, dist_mat, trial_id=None):
    ...
    print(f"[INFO] Using Strict-NoConf (Transformer retained, gat_dim={cfg['gat_dim']})")
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for X, y_mask, y_impact in train_loader:
            ...
            loss_bce = bce_fn(pred_mask, y_mask)
...
def main():
    ...
    def logging_callback(study, trial):
        if trial.value is not None:
            ...
            print(f"[Trial {trial.number:02d}] failed | params={trial.params}")
        print("-" * 80)

    study.optimize(objective, n_trials=20, callbacks=[logging_callback], catch=(Exception,))

    if len(study.trials) == 0 or all(t.value is None for t in study.trials):
        print("No valid trials completed. Please check model outputs.")
        return
    ...
    torch.save({
        ...
        "test_mse": test_mse,
        "test_acc": test_acc
    }, "logs/best_stgnn_raa_noconf_10_2000_seed42.pt")

    print(f"[OK] Saved full model checkpoint → best_stgnn_raa_noconf.pt")
    print(f"Total study time: {(time.time() - study_start)/60:.2f} min")
    # ⭐ 额外：对 test_loader 做一次全量诊断 dump（方便后续画图）
    diag_path = "logs/diagnostics_no_conf_10_2000_seed42.npz"
